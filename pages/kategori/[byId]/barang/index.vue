<template>
    <div>
        <div class="container mx-auto">
            <p class="font-semibold text-center mt-2 mb-5">Barang</p>
            <a :href="`/kategori/${kategoribyId}/adddata`"
                ...
                <span class="mt-1 ml-1"> Add new </span>
            </a>
            <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
                <table class="w-full text-sm text-left text-slate-500">
                    <thead class="bg-slate-100 text-xs border-b text-slate-700 uppercase hover:bg-slate-50 text-center">
                        <tr>
                            <th scope="col" class="px-6 py-4">Kategori id</th>
                            <th scope="col" class="px-6 py-4">Barang id</th>
                        ...
                        <tr v-for="(row, index) in data_barang" :key="row.kategoribyId"
                            ...
                            <td class="px-3 py-2 mx-auto">
                                <div class="grid lg:grid-cols-2 md:grid-cols-1 w-28 mx-auto">
                                    ...
                                    <button type="button" @click="($event) => deleteKategori($event, row.kategoribyId)"
...
export default {
    ...
    mounted() {
        this.kategoribyId = this.$route.params.byId;
        this.getBarang(this.kategoribyId);
    },
    methods: {
        getBarang(kategoribyId) {
            axios.get(BaseUrl + `/kategori/${kategoribyId}/barang`).then((res) => {
                this.data_barang = res.data;
            });
        },

        deleteKategori(event, kategoribyId) {
            if (confirm("Apakah anda yakin ingin menghapus data ini ?")) {
                event.target.innerText = "Deleting";
                axios.delete(BaseUrl + `kategori/${kategoribyId}`).then((res) => {
